...
def summary(a: np.ndarray) -> tuple[float, float, float, float]:
    ...
    # Calcula el valor máximo, mínimo, media y desviación típica de un array de numpy
    minimo = np.min(a)
    maximo = np.max(a)
    media = np.mean(a)
    desviacion = np.std(a)
    # Devuelve los resultados en una tupla
...
def ith(temperature: np.ndarray, humidity: np.ndarray) -> np.ndarray:
    ...
    if temperature.shape != humidity.shape:
        raise ValueError("Shape of data sensors must be the same. Temperature: {} != humidity: {}".
                         format(temperature.shape, humidity.shape))
    indice_th = np.round(0.8 * temperature + (humidity / 100) * (temperature - 14.3) + 46.4)
    return indice_th
...
def isStress(ith: np.ndarray) -> np.ndarray:
    ...
    estres = ith > 78  # Usamos una máscara booleana
